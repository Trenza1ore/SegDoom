from typing import Literal, Any, Dict, Generator, List, Optional, Tuple, Union
...
CompressionMethod = namedtuple("CompressionMethod", ["compress", "decompress"])

__compression_method_mapping = {
    "rle": CompressionMethod(compress=rle_compress, decompress=rle_decompress)
}
...
class CompressedRolloutBuffer(RolloutBuffer):
    ...
    def __init__(
        ...
        self.dtypes = dtypes or dict(element=np.uint8, length=np.uint8, position=np.uint16)
        self.compress, self.decompress = __compression_method_mapping[compression_method]
        self.reset()

    def reset(self) -> None:
        self.observations = np.zeros((self.buffer_size, self.n_envs, np.prod(self.obs_shape)),
                                     dtype=self.dtypes["element"])
        self.len_arr = np.empty()
        self.pos_arr = np.empty()
