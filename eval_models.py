...
def main():
    ...
    for config, name, input_rep, seed, additonal_config, model_path in tasks:
        ...
        if model_path:
            if not os.path.exists(f"./logs/{name}/{model_path}"):
        ...
        try:
            model = model_cls.load(f"./logs/{name}/{model_path}")
        except:
            try:
                model_cls = PPO
                model = model_cls.load(f"./logs/{name}/{model_path}")
        ...
        prob_alloc = max(mem - mem_after_model_alloc, 0.0)
        print(f"{mem_after_model_alloc:.2f} GB available, model & venv probably occupied {prob_alloc:.2f} GB")
        ...
        # Calling environment methods this way triggers warning from Gymnasium
        # But multi-processing is complicated and I don't want to reinvent the wheel
        # gymnasium\core.py:311: UserWarning: 
        # WARN: env.save to get variables from other wrappers is deprecated and will be 
        # removed in v1.0, to get this variable you can do `env.unwrapped.save` for 
        # environment variables or `env.get_wrapper_attr('save')` that will search the 
        # reminding wrappers.
